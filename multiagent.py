...
def main():
    st.title("Document Evaluation Workflow")
    ...
    if uploaded_file is not None:
        try:
            ...
            # Initialize state with file path
            initial_state = AgentState(file_path="temp_uploaded_file.pdf")
            ...
            #     st.write(f"Final state type: {type(final_state)}")

            #     # Print all attributes in the state
            #     st.write("Final state attributes:")
            #     for key, value in (
            #         final_state.items()
            #         if hasattr(final_state, "items")
            #         else vars(final_state).items()
            #     ):
            #         st.write(f"- {key}: {type(value)}")

            # Display classification results first
            st.subheader("Document Classification")
            if "classification" in final_state:
                st.write("Classification Results:")
                st.json(final_state["classification"])

                # Display predicted category
                if "predicted_category" in final_state:
                    st.info(f"Predicted Category: {final_state['predicted_category']}")
            ...
            if "evaluation" in final_state:
                ...
                st.warning("No evaluation results found in the final state.")
